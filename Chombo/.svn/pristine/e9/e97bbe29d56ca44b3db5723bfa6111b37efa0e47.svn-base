...
void EBLevelDataOps::getMaxMinOnProcessor(Real&                       a_max_val,
                                          Real&                       a_min_val,
                                          VolIndex&                   a_vof_max,
                                          VolIndex&                   a_vof_min,
                                          const LevelData<EBCellFAB>& a_data,
                                          ...
                                          const bool&                 a_do_abs)
{
  CH_TIME("EBLevelDataOps::getMaxMinLoc");
  //this function gets the max and min (valid) values
  a_max_val = -1.e99;
  a_min_val =  1.e99;
  ...
  for (DataIterator dit=a_data.dataIterator();dit.ok();++dit)
  {
    ...
    for (vofit.reset();vofit.ok(); ++vofit)
    {
      const VolIndex& cur_vof = vofit();
      Real data_val = dataEBFAB(cur_vof,a_comp);
      Real val_comp = data_val;
      if(a_do_abs)
      {
        val_comp = std::abs(data_val);
      }
      if(val_comp > a_max_val)
      {
        a_max_val = val_comp;
        a_vof_max = cur_vof;
      }
      if(val_comp < a_min_val)
      {
        a_min_val = val_comp;
        a_vof_min = cur_vof;
      }
    }//  end loop over vofs in box
  } //   end loop over boxes
}  //    end function getMaxMinOnProcessor
