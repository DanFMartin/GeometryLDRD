...

{
  ...
  pout() << "EBCMac::project: before kappaDivergence " << endl;
  kappaDivergence(m_divu, a_velocity, a_boundaryVelo);

  pout() << "EBCMac::project: before scale " << endl;
  EBAMRDataOps::scale(m_divu,a_divuCoef);

  pout() << "EBCMac::project: before initialize " << endl;
  ...
  bool zeroPhi = false;
  pout() << "EBCMac::project: before solve " << endl;
  ...
  pout() << "EBCMac::project: before gradient " << endl;
  gradient(a_gradient, m_phi);
  pout() << "EBCMac::project: before gradient BC " << endl;
  enforceGradientBC(a_gradient,m_phi);

  pout() << "EBCMac::project: before grad scale " << endl;
  EBAMRDataOps::scale(a_gradient,a_gradCoef);

  pout() << "EBCMac::project: before avgDown " << endl;
  ...
  if (m_subtractOffMean)
    {
      ...
      pout() << "EBCMac::project: before subtractOffMean " << endl;
      EBAMRDataOps::subtractOffMean(m_phi, m_eblg, m_refRat);
    }

  pout() << "EBCMac::project: before hodge decomposition " << endl;
  //u := u - grad phi
  for (int ilev = 0; ilev < m_numLevels; ilev++)
    {
  ...
  for (int ilev = 0; ilev < m_numLevels; ilev++)
    {
      if (ilev>0)
        {
          pout() << "EBCompMAC::gradient: before quadCFI" << endl;
      ...
      pout() << "EBCompMAC::gradient: before macGradient" << endl;
      ...
      if (ilev>0)
        {
          pout() << "EBCompMAC::gradient: before fillCoarseFineTangentialGradient" << endl;
              ...
              for(SideIterator sit; sit.ok(); ++sit)
                {
                  ...
                  ivsFlap &= boxFlap;
                  ivsFlap &= a_ebgraph.getDomain();
                  if(!ivsFlap.isEmpty())
                    {
                      
                      FaceIterator faceit(ivsFlap, a_ebgraph, velFaceDir, FaceStop::SurroundingNoBoundary);
                      for(faceit.reset(); faceit.ok(); ++faceit)
                        {
