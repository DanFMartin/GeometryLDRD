                        ...
                        bool ownData)
{
  ...
  if (!ownData)
    {
      // simple copy from input, copies pointers to AmrLevels
      m_amrData = amrData;
    }
  else      
    {
      // in case this is the first time through, define level 0, guard
      // against odd case where level 0 isn't defined
      if ((m_amrData.size() == 0) && (amrData.size() > 0))
        {
          m_amrData.resize(1);
          m_amrData[0] = new LevelData<FArrayBox>(amrData[0]->getBoxes(),
                                                  amrData[0]->nComp(),
                                                  amrData[0]->ghostVect());
          // copy from existing
          DataIterator dit = m_amrData[0]->getBoxes().dataIterator();
          for (dit.begin(); dit.ok(); ++dit)
            {
              (*m_amrData[0])[dit].copy((*amrData[0])[dit]);
            }
        }
    
          
      // clear existing storage -- assume we never change level 0
      for (int lev=1; lev< m_amrData.size(); lev++)
        {
          delete m_amrData[lev];
        }
      // in case we change the number of levels
      m_amrData.resize(amrData.size());
      for (int lev=1; lev < amrData.size(); lev++)
        {
          if (amrData[lev] != NULL)
            {
              m_amrData[lev]  = new LevelData<FArrayBox>(amrData[lev]->getBoxes(), amrData[lev]->nComp(), amrData[lev]->ghostVect());

              // copy from existing
              DataIterator dit = m_amrData[lev]->getBoxes().dataIterator();
              for (dit.begin(); dit.ok(); ++dit)
                {
                  (*m_amrData[lev])[dit].copy((*amrData[lev])[dit]);
            }
...
{
  CH_TIME("AmrAdaptor::printFile");

  // check for null file pointer, assume it's open
  CH_assert(outfile != NULL);
...
void AmrAdaptor::printFileHDF(const char* filename)
{
  ...
  Vector<string>  vectNames(m_comp);

  for (int ivar = 0; ivar < m_comp; ivar++)
    {
      char labelChSt[100];
      sprintf(labelChSt, "component_%d", ivar);
      string label(labelChSt);
      vectNames[ivar] = label;
    }
  ...
  // assemble the grids
  int numLevels = m_amrData.size();
  Vector<DisjointBoxLayout> vectGrids(numLevels);
  for (int lev=0; lev<numLevels; lev++)
    {
      vectGrids[lev] = m_amrData[lev]->getBoxes();
    }

  Box domain = vectGrids[0].physDomain().domainBox();
  // just in case
  if (!domain.ok())
    {
      IntVectSet domainIVS;
      LayoutIterator lit = vectGrids[0].layoutIterator();
      for (lit.begin(); lit.ok(); ++lit)
        {
          domainIVS |= vectGrids[0][lit];
        }
      domain = domainIVS.minBox();
    }
  ...
  WriteAMRHierarchyHDF5(fNameString,
                        vectGrids,
                        m_amrData,
                        vectNames,
                        domain,
