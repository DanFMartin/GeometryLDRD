  ...
  if (!m_doRestart)
    {
      ...
      copyPressureFromProjection();
     if (m_params.m_verbosity >= 2)
        {
          pout() << "Done with projection " << endl;
        }
      filter(m_velo);
      if (m_params.m_verbosity >= 2)
        {
          pout() << "Done with filter" << endl;
        }
...
copyPressureFromProjection()
{
  Interval sca_int(0,0);
  Vector<LevelData<EBCellFAB>* >& mac_phi_vec_ptr = m_macProjector->getPhi();
  for (int ilev = 0; ilev <= m_finestLevel; ilev++)
  {
    LevelData<EBCellFAB>& proj_phi_level = (*mac_phi_vec_ptr[ilev]);
    LevelData<EBCellFAB>& pressure_level =          (*m_pres[ilev]);
    
    proj_phi_level.copyTo(sca_int, pressure_level, sca_int);
  }
}
/*****************/
void
