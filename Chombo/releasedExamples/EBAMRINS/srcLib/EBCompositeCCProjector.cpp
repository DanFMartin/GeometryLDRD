...

{
  ...
  pout() << "EBCompCCP::project:before mac project" << endl;
  const int status = m_macProjector->project(m_macVelo, m_macGrad, a_gradCoef, a_divuCoef, a_boundaryVelo);

  pout() << "EBCompCCP::project:before average face to cells" << endl;
  averageFaceToCells(a_gradient, m_macGrad);

  pout() << "EBCompCCP::project:before average incr" << endl;
  EBAMRDataOps::incr(a_velocity, a_gradient, -1.0);
  pout() << "EBCompCCP::project:before return" << endl;
