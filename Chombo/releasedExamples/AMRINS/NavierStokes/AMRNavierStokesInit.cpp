...
AMRNavierStokes::initialData()
{
  ...
  if (s_init_vel_from_vorticity)
  ...
  else
    {
      ...
      // Defaults are standard Taylor-Green spindown problem
      Real omega0=1.0;
      Real rad0 = 0.25;
      RealVect center0 = 0.5*RealVect::Unit;

      ParmParse ppInit("init");
      ppInit.query("omega", omega0);
      ppInit.query("radius", rad0);
      if (ppInit.contains("center"))
        {
          Vector<Real> centerVect(SpaceDim);
          ppInit.getarr("center", centerVect, 0, SpaceDim);
          center0 = RealVect(D_DECL(centerVect[0],centerVect[1],centerVect[2]));
        }
