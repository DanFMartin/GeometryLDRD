...
testVectorOps(SUNContext ctx) {
  ...
  */
  ...
  if (false)
    N_VPrint(v); // can diff this stdout against the file nvecAmr.txt

  // Check print using old school C interface
  std::string filename = "nvecAmr.txt";
  FILE* fp = fopen(filename.c_str(), "w");
  assert(fp != NULL);
  pout() << "Testing print to file: " << filename 
    << ", should have " << numAmrPts << " lines" << endl;
  N_VPrintFile(v,fp);
  fclose(fp);
  // TODO - Check the file contents or compare to regression?
