...
main(int argc, char** argv)
{
...

  {
    ...
    // check that deepBL are equivalent, but have different refCountedPtrs 
    if (!(deepBL == plan1) || (deepBL.ptrEqual(plan1)))
    {
      pout() << indent << pgmname
  ...
  // similar test, but with anisotropic refinement ratios
  DisjointBoxLayout refinedPlan2;
  IntVect refRatio(IntVect(D_DECL6(2,4,8,2,4,8)));
  refine(refinedPlan2, plan1, refRatio);
  CH_assert(refinedPlan2.isClosed());

  DisjointBoxLayout recoarsenedPlan2;
  coarsen(recoarsenedPlan2, refinedPlan2, refRatio);
  CH_assert(recoarsenedPlan2.isClosed());

  for (LayoutIterator it(plan1.layoutIterator()); it.ok(); ++it)
    {
      if (plan1.get(it()) != recoarsenedPlan2.get(it()))
        {
          pout() << indent << pgmname
               << " FAILED: anisotropic refine() then coarsen() does not seem to be reversible." << endl ;
          success = false ;
          if (verbose)
            pout() << indent2 << plan1[it()] << endl;
        }
  ...
  // test setting Copier before exchange
  Copier testCopier(level1.getBoxes(), level1.getBoxes(), level1.ghostVect(), true);
  level1.setExchangeCopier(testCopier);
  level1.exchange();
  Copier blankCopier;
  level1.setExchangeCopier(blankCopier);
  level1.exchange();
