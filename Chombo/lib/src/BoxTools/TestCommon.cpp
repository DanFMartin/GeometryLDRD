...
buildDisjointBoxLayout(DisjointBoxLayout& plan,const IntVectSet& tags, const Box& domain)
{
  ...
  // basic non-periodic problemDomain 
  ProblemDomain physDomain(domain);
  ...
  int bufferSize = 0;
  int blockFactor = 1;
  BRMeshRefine mr(physDomain, fakeNRef, fillRatio, blockFactor,
                  bufferSize, maxsize);

  mr.makeBoxes(vectBox, tags, domainivs, physDomain, maxsize, 0);
  ...
  plan.define(vectBox, assignments, physDomain);
